...
%D on le calcul nous meme
D= double(imread('Images/grenouille_avec_texte.png'));
masque_non_jaune = ~(D(:,:,1) == 255 & D(:,:,2) == 255 & D(:,:,3) == 0);
masque_aplati = masque_non_jaune(:);
W_D = spdiags(masque_aplati, 0, nb_pixels, nb_pixels);
...
% Point fixe :
lambda = 0.01;			% Poids de la regularisation
u_k = u_0;
convergence = +Inf;
iteration = 0;
...
while convergence > 1e-3
	...
	% Iteration (6) :
	u_kp1 = inpainting(b,u_k,lambda,Dx,Dy,epsilon, W_D);	
