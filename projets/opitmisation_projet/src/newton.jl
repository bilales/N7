...
function newton(f::Function, gradf::Function, hessf::Function, x0::Union{Real,Vector{<:Real}}; 
   ...
   #
   norm_gradf_x0 = norm(gradf(x0))
   if norm_gradf_x0 <= max(tol_rel * norm_gradf_x0, tol_abs)
      return x0, f(x0), 0, 0, [x0]
   end
   ...
   while true
      ...
      dk = -hessf(xs[k])\gradf(xs[k])

      #on ajoute le vecteur xk+1
      xs = vcat(xs, [xs[k]+dk])

      #condtion 1
      if (norm(gradf(xs[k+1])) <=max(tol_rel*norm_gradf_x0, tol_abs))
         flag = 0
         break
      end

      #condition 2
      if (norm(xs[k+1]-xs[k]) <= epsilon*max(tol_rel*norm(xs[k]), tol_abs))
         flag = 1
         break
      end

      #condition 3
      if (abs(f(xs[k+1])-f(xs[k])) <= epsilon*max(tol_rel*abs(f(xs[k])), tol_abs))
         flag = 2
         break
      end 
