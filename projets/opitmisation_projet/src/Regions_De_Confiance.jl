...
function regions_de_confiance(f::Function, gradf::Function, hessf::Function, x0::Vector{<:Real};
    ...
    while (nb_iters<=max_iter)
        ...
        # Calcul du pas
        if (algo_pas=="cauchy")
            sk = cauchy(gradf(xk), hessf(xk), Δk;tol_abs)
        elseif (algo_pas=="gct")
            sk = gct(gradf(xk), hessf(xk), Δk, max_iter=max_iter_gct,tol_abs=tol_abs,tol_rel=tol_rel)
        end

        # Calcul du modèle
        m(s)=f(xk)+gradf(xk)'*s+(1/2)*s'*hessf(xk)*s
        # Calcul du rhok
        rhok = (f(xk)-f(xk+sk))/(m(zero)-m(sk))
        ...
        # Mise à jour du flag
        if (norm(gradf(xk_1)) <= max(norm(gradf(x0))*tol_rel,tol_abs))
            flag = 0
            break   
