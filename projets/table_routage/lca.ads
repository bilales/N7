...
package LCA is
	...
	-- supprime une cellule au début
	procedure Supprimer_debut (Sda : in out T_LCA) with
		Post =>  Taille (Sda) = Taille (Sda)'Old - 1;


	--Permet d'accéder à la clé et à la donnée souhaité en fonction de l'indice, une procédure similaire à celle d'accéder à l'élément i d'un tableau, i 
	procedure Indice_Cellule (Sda : in T_LCA; Indice : in Integer; Cle : out T_Cle; Donnee : out T_Donnee);
