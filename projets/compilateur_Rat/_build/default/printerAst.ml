...
struct
  ...
  let string_of_binaire b =
    ...
    let rec string_of_affectable a = 
      match a with
      |Ident n -> n^" "
      |Deref v -> "*" ^(string_of_affectable v)^""
      |AccesTableau (v,e) -> (string_of_affectable v)^"["^(string_of_expression e)^"] "
  ...
  and string_of_expression e =
    ...
    | Adresse ad -> "& "^ad^" "
    | CreationTableau (t,e) -> "new "^(string_of_type t)^"["^(string_of_expression e)^"] "
    | InitialisationTableau (le) -> "{"^((List.fold_right (fun i tq -> (string_of_expression i)^tq) le ""))^"} "
